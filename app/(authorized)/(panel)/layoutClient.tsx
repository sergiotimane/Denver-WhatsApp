...
const ContactTabContext = React.createContext(null);

export function useSetContactTab() {
...
export default function PanelClient({ children }: { children: ReactNode }) {
	...
	return (
		<div className={`flex flex-col h-screen ${contactTab ? 'show-contacts' : ''}`}>
			<div className='h-16 flex flex-row justify-between px-4 flex-shrink-0'>
				<div className='flex flex-row'>
					<div className='flex flex-row gap-2 items-center'>
						<button onClick={() => setContactTab((pstate) => !pstate)} id='open-contacts' className='max-sm:block hidden h-8 w-8 rounded-md bg-gray-200'>
						...
						<img src='/assets/img/Payslips.png' className='h-8' />
					</div>
				</div>
				{/* <div className="flex flex-row"> */}
			...
			{/* <div className="flex-[6] max-w-xs border-e-2 border-e-slate-100 p-4">
                <div className="text-center">Fixonet Whatsapp</div>
                <div className="mt-8">
                </div>
            </div> */}
			<div className='h-full overflow-y-auto bg-gray-100 flex-grow'>
				<ContactTabContext.Provider value={setContactTab}>{children}</ContactTabContext.Provider>
			</div>
		</div>
	);
}
